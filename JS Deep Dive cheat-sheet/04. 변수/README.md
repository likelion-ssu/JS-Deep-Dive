...
하지만 단축하여 사용해도 선언부는 런타임 이전에 호이스팅되고, 변수 할당은 런타임에 분리되어 실행된다.
...
- var 키워드로 선언한 변수는 선언과 동시에 `undefined`로 초기화되기 때문에 엄밀히 말하면 처음으로 값을 할당하는 것도 재할당이다.
  > ES6에서 도입한 const 키워드를 사용하면 해당 변수는 재할당이 금지되어 상수를 표현할 수 있다.
- 기존 메모리 공간의 값을 지우고 새 값을 저장하는 것이 아니라, 새로운 메모리 공간을 확보하여 저장한다.
- 식별자와 연결이 끊어진 기존 값은 가비지 콜렉터\*에 의해 자동으로 해지되며, 해지 시점은 예측할 수 없다.
